...
class UsersToJson:
    ...
    def load_users_from_file_to_db(self):
        ...
        for complete_user_dict in complete_users:
            self.save_user(complete_user_dict)
        return None
    ...
    def save_user_password(self, user, password):
        ...
        try:
            cursor.execute("""UPDATE users SET first_name=%s WHERE id=%s""", ('bb king', user.id))
            cursor.close()
            connection.commit()
            user.save()
        except Exception as e:
    ...
    def save_user(self, complete_user_dict):
        success = True
        user = complete_user_dict['user']
        email = user['email']
        if self.is_user_in_db(email):
            print 'user with email: ' + email + ' is already in database'
            success = False
        else:
            print email + ' is not in database. Saving new user.'
            banter_user = complete_user_dict['banter_user']
